...
import { X, Printer, PackageOpen, Receipt as ReceiptIcon, FileText, ArrowLeft } from 'lucide-react';
            ...
            <div className="bg-white rounded-sm shadow-2xl max-w-md w-full max-h-[90vh] overflow-auto">
                ...
                <div ref={receiptRef} className="receipt-content p-4">
                    ...
                    <div className="text-center border-b border-dashed border-gray-400 pb-3 mb-3">
                        {company.logo && (
                            <img src={company.logo} alt="Logo" className="h-12 mx-auto mb-2 grayscale" />
                        )}
                                ...
                                {transaction.items.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="py-2 text-right align-top">
                                            {formatRowPrice(item.isBulk ? item.bulkPrice : item.price)}
                                        </td>
                                        <td className="py-2 text-right font-medium align-top">
                                            {formatRowPrice((item.isBulk ? item.bulkPrice : item.price) * item.inputQuantity)}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        ...
                        {transaction.paymentMethod === 'cash' && (
                            <>
                                <div className="flex justify-between">
                                    ...
                                    <span>{formatPrice(transaction.amountGiven || transaction.total)}</span>
                                </div>
                    ...
                    <div className="text-center pt-3 border-t border-dashed border-gray-400 text-xs">
                        <div className="space-y-1">
                            <p className="font-medium whitespace-pre-wrap">{company.receiptMessage}</p>
                        </div>
                    </div>
