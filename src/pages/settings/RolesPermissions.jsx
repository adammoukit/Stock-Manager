    ...
    return (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-[#018f8f] tracking-tight">Rôles & Droits d'Accès</h2>
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-sm transition-colors shadow-md font-medium ${selectedRoleId === 'admin'
                        ...
                        : 'bg-[#018f8f] text-white hover:bg-teal-700'
                        }`}
                ...
                <div className="md:col-span-1 space-y-2">
                    <div className="bg-white p-3 rounded-sm border-2 border-gray-300 shadow-sm mb-4">
                        <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                            <Info className="w-4 h-4 text-[#018f8f]" />
                    ...
                    {roles.map(role => (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className={`font-bold ${selectedRoleId === role.id ? 'text-[#018f8f]' : 'text-gray-700'}`}>
                                    {role.name}
                                </span>
                            </div>
                        </button>
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`p-4 rounded-sm border-2 flex items-start justify-between cursor-pointer transition-all ${isGranted
                                                        ? isSensitive ? 'bg-orange-50 border-orange-200' : 'bg-teal-50 border-teal-200'
                                                        : 'bg-gray-50 border-gray-200 hover:border-gray-300'
                                                        } ${selectedRoleId === 'admin' ? 'cursor-not-allowed opacity-80' : ''}`}
                                                >
                                                    <div className="flex-1 pr-4">
                                                        <div className={`font-medium text-sm mb-1 ${isGranted ? (isSensitive ? 'text-orange-800' : 'text-teal-800') : 'text-gray-600'}`}>
                                                            {permLabel}
                                                        </div>
                                                        {isSensitive && (
                                                    ...
                                                    {/* Toggle Switch UI */}
                                                    <div className={`relative inline-flex h-7 w-[50px] items-center rounded-full transition-colors flex-shrink-0 ${isGranted ? 'bg-[#018f8f]' : 'bg-red-400'}`}>
